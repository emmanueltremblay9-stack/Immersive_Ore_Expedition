...
Province System v20 adds declaration-only configured and placed feature JSON resources for `immersive_ore_expedition:tiny_vertical_mine_entrance`, pointing at the v19 custom feature type with empty `NoneFeatureConfiguration` config and an empty placement modifier list. v20 adds no new config keys and changes no defaults. It does not add a biome modifier, does not bind the placed feature to any biome, does not add blocks, items, entities, ores, gems, dependencies, or live gameplay-loop placement, and default configs remain safe/no-op.

Province System v21 adds a default-off biome modifier smoke-tag bridge for the same placed feature. It adds no new config keys and changes no defaults. The shipped biome modifier targets only `#immersive_ore_expedition:worldgen_smoke_test_biomes`, and the shipped tag contains zero biome ids, so no real biome receives the placed feature by default. Future controlled smoke profiles may opt in with an external datapack that appends an explicit test biome to that tag, but both `worldgen.runtimeProofFeatureEnabled` and `worldgen.runtimePlacementEnabled` still default to `false` and must be enabled before the v19/v18 proof path can attempt placement. v21 does not add blocks, items, entities, ores, gems, dependencies, broad biome bindings, or full gameplay-loop proof.
